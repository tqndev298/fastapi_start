...
async def update_ticket_details(
    ...
    ticket_query = update(TicketDetails).where(TicketDetails.id == ticket_id)
...
async def get_event_sponsorships_with_amount(db_session: AsyncSession, event_id: int):
    query = (
        ...
        # .join(Sponsorship, Sponsorship.id == Sponsor.id)
        # .join(Event, Sponsorship.event_id == Event.id)
        # .order_by(
        #     Sponsorship.amount.desc(),
        # )
        select(Sponsor.name, Sponsorship.amount)
        .join(Sponsorship, Sponsorship.id == Sponsor.id)
...
async def sell_ticket_to_user(
    ...
    query = (
        update(Ticket)
        .where(and_(Ticket.id == ticket_id, Ticket.sold == False))
